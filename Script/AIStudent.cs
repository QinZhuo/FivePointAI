...
public class AIStudent : AiLv2
{
    SortedList<string,float> remeberStep1;
    SortedList<string, float> remeberStep2;
    // Use this for initialization
    void Start () {
        remeberStep1 = new SortedList<string, float>() ;
        remeberStep2 =new SortedList<string, float>();
        if (chessColor != Chess.观战) Debug.Log(chessColor + "AI: 记忆AI");
        ...
        toScore.Add("_aa_a_", 5000);                     //跳活三
        toScore.Add("_a_aa_", 5000);
        ...
        foreach (var strkey in toScore)
        {
            //toScore[strkey.Key] = 0;
            if (PlayerPrefs.HasKey(strkey.Key))
            {
                
                toScore[strkey.Key] = PlayerPrefs.GetFloat(strkey.Key);
                Debug.Log("存在" + strkey.Key + ":" + toScore[strkey.Key]);
            }
            remeberStep1.Add(strkey.Key, 0);
            remeberStep2.Add(strkey.Key, 0);
        }
    }

    public void SetScoreRemeber(int[] pos,int chess)
    {
        SetOneLineRemeber(pos, new int[2] { 0, 1 }, chess);
        SetOneLineRemeber(pos, new int[2] { 1, 0 }, chess);
        SetOneLineRemeber(pos, new int[2] { 1, 1 }, chess);
        SetOneLineRemeber(pos, new int[2] { -1, 1 }, chess);
        //Debug.Log("记录：[" + (Chess)chess + "]" + pos[0] + "," + pos[1] + "点信息");
    }

    public void SetOneLineRemeber(int[] pos, int[] add, int chess)
    {
        ...
        if (cmpTrue != "")
        {
           // Debug.Log(cmpTrue + ":+1");
            if (chess == 1)
            {
                remeberStep1[cmpTrue]+=1;
            }
            else
            {
                remeberStep2[cmpTrue]+=1;
            }
    ...
    public void ClearRemeber()
    {
        PlayerPrefs.DeleteAll();
    }

    public void RemeberWiner(int chess)
    {
        ...
        if (chess == 1)
        {
            foreach (var strkey in toScore)
            {
                toScore[strkey.Key] *= ((remeberStep1[strkey.Key] * 5-remeberStep2[strkey.Key]*5)+100.0f)/100.0f;
                PlayerPrefs.SetFloat(strkey.Key, toScore[strkey.Key]);
                
            }
            Debug.Log("记录：[" + (Chess)chess + "]" +"胜利步骤调整评分");
        }
        else if(chess==2)
        {
            foreach (var strkey in toScore)
            {
                ...
                toScore[strkey.Key] += ((remeberStep2[strkey.Key] * 5 - remeberStep1[strkey.Key] * 5) + 100.0f) / 100.0f;
                //Debug.Log((remeberStep2[strkey.Key]+" "+ remeberStep1[strkey.Key]+" "+ toScore[strkey.Key]));
                PlayerPrefs.SetFloat(strkey.Key, toScore[strkey.Key]);
